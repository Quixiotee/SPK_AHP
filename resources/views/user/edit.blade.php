...
<div class="pd-20 card-box mb-30">
    <div class="clearfix">
        <div class="pull-left">
            ...
            <p class="mb-30">Edit User</p>
        </div>
    </div>
    <form action="/user/{{$user->id_user}}/edit" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        @method('put')
        <div class="form-group row">
            <label class="col-sm-12 col-md-2 col-form-label">Id User</label>
            <div class="col-sm-12 col-md-10">
                <input class="form-control" type="number" name="id_user" readonly>
            </div>
        </div>
        <div class="form-group row">
        ...
        <div class="form-group row">
            ...
            <select class="form-control" id="exampleFormControlSelect1" name="id_pegawai">
                    ...
                    <option value="{{$row->id_pegawai}}" @if($row->id_pegawai == $user->id_pegawai) selected @endif>{{$row->id_pegawai}}</option>
                    @endforeach
                </select>
            </div>
        </div>
